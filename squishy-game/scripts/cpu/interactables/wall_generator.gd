...
@export var slit_edge_angle_degrees: float = 12.0
...
func _spawn_wall() -> void:
	...
	var side_width: float = max((wall_width - slit_width) * 0.5, 0.1)
	var side_offset: float = slit_width * 0.5 + side_width * 0.5

	wall_parts.append(_create_wall_part(
		"VerticalSlitWallLeft",
		-_get_side_direction() * side_offset,
		Vector3(side_width, wall_height, wall_thickness),
		wall_height * 0.5,
		1.0
	))
	wall_parts.append(_create_wall_part(
		"VerticalSlitWallRight",
		_get_side_direction() * side_offset,
		Vector3(side_width, wall_height, wall_thickness),
		wall_height * 0.5,
		-1.0
...
func _create_wall_part(part_name: String, side_offset: Vector3, size: Vector3, center_height: float, slit_angle_sign: float) -> Rigidbody:
	...
	var spawn_position := global_position + side_offset
	spawn_position.y = _get_ground_height() + center_height
...
func _get_ground_height() -> float:
	...
	return groundNode.global_position.y
